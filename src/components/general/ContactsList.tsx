...
export const ContactsList: React.FC<ContactsListProps> = ({ friends, loading = false, onChatClick, refreshUnreadTrigger }) => {
  ...
  const { onlineUserIds, chatUnreadBySenders, refetchChatUnread } = useSharedData();
  const currentUserId = session?.user?.id;
  const currentUserEmail = session?.user?.email;
  const [unreadDelta, setUnreadDelta] = useState<Record<string, number>>({});

  // Merge shared unread dengan local delta (real-time new messages)
  const unreadBySender = React.useMemo(() => {
    const merged = { ...chatUnreadBySenders };
    Object.entries(unreadDelta).forEach(([k, v]) => {
      merged[k] = (merged[k] ?? 0) + v;
    });
    return merged;
  }, [chatUnreadBySenders, unreadDelta]);

  useEffect(() => {
    if (refreshUnreadTrigger !== undefined && refreshUnreadTrigger > 0) {
      setUnreadDelta({}); // Clear local delta, refetch akan dapat count terbaru dari server
      refetchChatUnread();
    }
  }, [refreshUnreadTrigger, refetchChatUnread]);
  ...
  useWebSocketSubscription((data: any) => {
    ...
    if (payload && currentUserId && payload.receiver_id === currentUserId) {
      ...
      setUnreadDelta((prev) => ({ ...prev, [senderId]: (prev[senderId] ?? 0) + 1 }));
    }
  });
